...
ACCEL_THRESHOLD = 2.5  # G-force threshold for a fall (in terms of acceleration magnitude, in Gs)
GYRO_THRESHOLD = 300  # Threshold for angular velocity in degrees per second (to detect sudden rotation)
...
ACCEL_VISUAL_THRESHOLD = 2.5  # For visualizing the upper threshold on the plot
        ...
        if line:
            try:
                ...
                if accel_magnitude > ACCEL_THRESHOLD:
                    # High acceleration detected, check if it's consistent over time
                    if high_accel_start_time == 0:
                        ...
                        # High acceleration sustained, now check for fall
                    ...
                    # Reset high acceleration time if below threshold
                    high_accel_start_time = 0

                # Monitor for post-fall inactivity
                if post_fall_monitoring:
                    if current_time - fall_start_time > POST_FALL_TIME_WINDOW:
                        # Check if there is no significant movement (both accel and gyro) after a fall
                        if accel_magnitude < 1.0 and gyro_magnitude < 50:
...
fig, ax = plt.subplots()
accel_line, = ax.plot([], [], label='Accelerometer Magnitude', color='blue', linewidth=1)
gyro_line, = ax.plot([], [], label='Gyroscope Magnitude', color='orange', linewidth=1)
accel_threshold_line, = ax.plot([], [], color='red', linestyle='--', linewidth=1, label='Accel Threshold')
gyro_threshold_line, = ax.plot([], [], color='green', linestyle='--', linewidth=1, label='Gyro Threshold')
