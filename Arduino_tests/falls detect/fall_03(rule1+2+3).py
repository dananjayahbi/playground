    ...
    while True:
        ...
        if line:
            try:
                ...
                gyro_magnitude_data.append(gyro_magnitude)  # Store gyro magnitude
...
gyro_line, = ax.plot([], [], label='Gyroscope Magnitude (degrees/s)', color='orange', linewidth=1)
...
def init():
    ...
    ax.set_ylabel('Magnitude (Gs & degrees/s)')
    ax.set_xlabel('Time (arbitrary units)')
    ax.legend()
    ax.margins(y=0.1)
    plt.subplots_adjust(bottom=0.15, left=0.15, right=0.95, top=0.85)
    return accel_line, gyro_line, accel_threshold_line, stationary_threshold_line, fall_detected_text
...
def update_plot(frame):
    if len(time_data) > 0:
        ...
        # Adjust y-axis dynamically to include the accel and gyro data
